...
import {
  Rocket, Trophy, Target, Clock, Activity, ArrowRight, FileText, Zap, Calendar, Crown, Medal, Sun, Moon, Bell, Search, ChevronLeft, ChevronRight, BookOpen, Layers, Code, Briefcase, ExternalLink, X, LogOut, HelpCircle, MessageCircle, FileQuestion, Send
} from 'lucide-react';
...
const Dashboard = () => {
  const { user, logout } = useAuth(); // Assuming you might want to use real user data later
                            ...
                            {chatMessages.map((msg) => (
                                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${
                                        ...
                                        <p>{msg.text}</p>
                                        <p className={`text-[10px] mt-1 ${msg.sender === 'user' ? 'text-teal-200' : 'text-gray-400'}`}>{msg.timestamp}</p>
                                    </div>
                                </div>
                            ))}
                            <div ref={chatEndRef} />
                ...
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-teal-100 to-blue-100 dark:from-teal-900/40 dark:to-blue-900/40 border border-white/20 dark:border-white/10 p-8 backdrop-blur-md shadow-xl dark:shadow-2xl">
                    ...
                    <div className="relative z-10">
                        <h2 className="text-4xl md:text-5xl font-bold mb-4 text-gray-900 dark:text-white">
                            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-blue-600 dark:from-teal-400 dark:to-blue-400">{mockData.user.name}</span>!
                        </h2>
                        <p className="text-xl text-gray-700 dark:text-gray-300 mb-6 max-w-2xl leading-relaxed">
                            You are <span className="text-emerald-600 dark:text-emerald-400 font-bold text-2xl">{mockData.user.readiness}%</span> ready for your dream job. 
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                {mockData.stats.map((stat, index) => (
                    <motion.div 
                        ...
                        className="col-span-1 md:col-span-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-5 backdrop-blur-sm hover:shadow-lg dark:hover:bg-white/10 transition-all hover:-translate-y-1 group shadow-sm"
                    >
                ...
                <motion.div variants={itemVariants} className="col-span-1 md:col-span-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-6 backdrop-blur-sm flex flex-col min-h-[350px] shadow-sm">
                    ...
                    <div className="flex-1 w-full h-full -ml-4">
                        <ResponsiveContainer width="100%" height="100%">
                            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={mockData.skills}>
                ...
                <motion.div variants={itemVariants} className="col-span-1 md:col-span-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-6 backdrop-blur-sm flex flex-col shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        {filteredRecentActivity.map((activity) => (
                            <div key={activity.id} className="group flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 border border-gray-100 dark:border-white/5 hover:border-gray-200 dark:hover:border-white/20 transition-all cursor-pointer">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <ArrowRight size={16} className="text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors" />
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <motion.div variants={itemVariants} className="col-span-1 md:col-span-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-6 backdrop-blur-sm flex flex-col min-h-[350px] shadow-sm">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-white/5">
                        <p className="text-xs text-gray-500 flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-pink-500" /> Next: System Design (25th)</p>
                    </div>
                </motion.div>

                {/* G. Study Resources */}
                <motion.div variants={itemVariants} className="col-span-1 md:col-span-12 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-6 backdrop-blur-sm flex flex-col shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
